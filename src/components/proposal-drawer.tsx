              ...
              <div className="p-6 sm:p-10">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="flex items-center gap-1.5 text-xs font-bold text-emerald-400">
                      ...
                      پیشنهادنامه آماده ارسال — اولویت {data.proposal.grade}
                    </p>
                    <h2 className="mt-2 text-2xl font-black leading-snug">
                      پیشنهاد ارتقای رتبه گوگل
                      <br />
                      <span className="text-emerald-300">{data.company.name}</span>
                    </h2>
                  </div>
                  <button
                    onClick={onClose}
                ...
                <div className="mt-4 flex flex-wrap gap-2 text-xs">
                  ...
                  <span className="rounded-full bg-white/5 px-3 py-1.5 text-zinc-300 ring-1 ring-white/10">
                    رتبه فعلی: {data.company.googleRank ? faNum(data.company.googleRank) : "—"}
                  </span>
                  {data.audit && (
                    <span className="rounded-full bg-white/5 px-3 py-1.5 text-zinc-300 ring-1 ring-white/10">
                      امتیاز سایت: {faNum(data.audit.score)}/۱۰۰
                    </span>
                  )}
                </div>
                ...
                {/* -------- 6. Pricing package -------- */}
                {data.proposal.pricing.length > 0 && (
                  <motion.section
                    initial={{ opacity: 0, y: 16 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true, margin: "-40px" }}
                  ...
                  >
                    ...
                    <p className="mt-2 text-xs leading-6 text-zinc-500">
                      {faNum(data.proposal.pricing.length)} بخش خدماتی — هر بخش با جزئیات کار و بازه هزینه واقعی بازار (تومان):
                    </p>
                    <div className="mt-4 space-y-2">
                      {data.proposal.pricing.map((p, i) => (
                        <div
                          key={p.title}
                    ...
                    <div className="mt-3 flex items-center justify-between rounded-2xl bg-amber-400 px-5 py-4 text-amber-950 shadow-lg shadow-amber-400/25">
                      ...
                      <span className="text-sm font-black tabular-nums">
                        {faNum(data.proposal.totalMin ?? 0)} تا {faNum(data.proposal.totalMax ?? 0)} تومان
                      </span>
                    </div>
                ...
                {data.proposal.penalties.length > 0 && (
                  <motion.section
                    initial={{ opacity: 0, y: 16 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true, margin: "-40px" }}
                  ...
                  >
                    ...
                    <div className="mt-4 space-y-2">
                      {data.proposal.penalties.map((pen) => (
                        <div
                          key={pen.title}

...
- No Python or JavaScript packages are required for the core app. Direct server-rendered PDF download is enabled when Pillow with RAQM is available; otherwise browser Print / Save as PDF remains available.
...
```text
clinic-lead-agent/
├── index.html                 # Clean HTML shell; no application code embedded
├── static/
│   ├── app.js                 # UI, scoring, proposals, partners and messaging
│   └── styles.css             # Responsive dashboard styles
├── server.py                  # Static server, audit, PDF and messaging APIs
├── Dockerfile                 # Hugging Face Spaces / Docker deployment
├── requirements.txt           # Pillow for direct Persian PDF generation
├── .dockerignore
├── smoke_test.py              # Health, assets, security, PDF and API tests
├── DEPLOYMENT_BLUEPRINT.md    # Architecture and free-hosting guide
